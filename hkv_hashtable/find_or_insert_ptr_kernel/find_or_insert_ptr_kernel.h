 ...
 */
 ...
 LAUNCH_BOUND(THREAD_NUM_512) inline void find_or_insert_ptr_kernel_vf(
     ...
     uint32_t bucket_capacity, uint32_t dim, __gm__ const K* keys_gm,
     __gm__ void* value_ptrs_gm, __gm__ S* scores, __gm__ K * __gm__* key_ptrs_gm, uint64_t n,
   ...
   for (uint64_t kv_idx = blockIdx * blockDim.x + threadIdx.x; kv_idx < n;
     ...
     __gm__ VecV* bucket_values_ptr = reinterpret_cast<__gm__ VecV*>(reinterpret_cast<__gm__ void*>(bucket->vectors));
     ...
     for (int offset = 0; offset < bucket_capacity; offset++) {
       ...
       if (key_val == key) {
         ...
         K try_key = Simt::AtomicCas(reinterpret_cast<__gm__ K*>(current_key_ptr),
