 ...
 */
 ...
 LAUNCH_BOUND(THREAD_NUM_2048) inline void dump_kernel_if_vf(
   ...
   for (; tid < search_length_align; tid += total_thread_num) {
     ...
     if (tid < search_length) {
      ...
      __gm__ VecV* bucket_values_ptr = reinterpret_cast<__gm__ VecV*>(bucket->vectors);
      ...
      if (match) {
        ...
        block_tuples[local_index].value = reinterpret_cast<uint64_t>(value_ptrs);
       ...
       if (cg_flag == 1) {
         ...
         for (uint32_t k = cg_rank_id; k < dim_in; k += GROUP_SIZE) {
           __stg<ST_L2CacheType::L2_CACHE_HINT_NORMAL_FV,
             ...
             d_vecv_val + val_start * dim_in + k, value_sync[k]);
         }
       }
     }
   }
 ...
 __global__ __vector__ void dump_kernel_if(
   ...
   __ubuf__ size_t* ub_shared_global_acc_mem =
       reinterpret_cast<__ubuf__ size_t*>(
           shared_global_acc_tensor.GetPhyAddr());

   DISPATCH_VALUE_SIZE(
       value_size,
       (asc_vf_call<dump_kernel_if_vf<K, V, S, DTYPE, PredFunctor, GROUP_SIZE>>(
           dim3{static_cast<uint32_t>(THREAD_NUM_2048)}, table, buckets,
           pattern, threshold, keys, vals, scores, offset, search_length,
