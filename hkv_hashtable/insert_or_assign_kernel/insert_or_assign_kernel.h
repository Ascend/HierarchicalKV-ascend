 ...
 */
   ...
   for (uint64_t kv_idx = block_index * blockDim.x + threadIdx.x;
     ...
     for (int32_t i = 0; i < group_size; i++) {
       ...
       if ((res_sync != OccupyResult::REFUSED &&
         ...
         uint64_t value_ddr_sync = asc_shfl(bucket_values_uintptr, i, group_size);
         ...
         for (uint32_t j = cg_rank_id; j < dim; j += group_size) {
           __stg<ST_L2CacheType::L2_CACHE_HINT_NORMAL_FV,
                 L1CacheType::NON_CACHEABLE>(
               reinterpret_cast<__gm__ VecV*>(value_ddr_sync) + bucket_value_start + j,
 ...
 __global__ __vector__ void insert_or_assign_kernel(
     ...
     uint32_t bucket_max_size, uint32_t dim, __gm__ void* keys, __gm__ void* values,
     __gm__ void* scores, uint64_t n, uint64_t global_epoch, uint32_t value_size, 
   ...
   uint64_t cur_score =
    ...
    DISPATCH_VALUE_SIZE(
           value_size,
           (asc_vf_call<insert_or_assign_kernel_with_digest_vf<
             K, V, S, DTYPE, Strategy>>(
           dim3{THREAD_NUM_512}, buckets,
 ...
 __global__ __vector__ void insert_or_assign_kernel_with_thread_1024(
   ...
   uint64_t cur_score =
    ...
    DISPATCH_VALUE_SIZE(
           value_size,
           (asc_vf_call<insert_or_assign_kernel_with_digest_vf_1024<
             K, V, S, DTYPE, Strategy>>(
           dim3{THREAD_NUM_1024}, buckets,
           buckets_size, capacity, bucket_max_size, dim, keys, values,
...
__global__ __vector__ void upsert_kernel_lock_key_hybrid(
  ...
  asc_vf_call<upsert_kernel_lock_key_hybrid_vf<K, V, S, Strategy>>(
      ...
      bucket_max_size, dim, keys, scores, cur_score, n, global_epoch,
      GetBlockIdx(), max_bucket_shift, capacity_divisor_magic,
      capacity_divisor_shift, n_align_warp, thread_all, d_dst_values,
      d_dst_keys);
}
...
__global__ __vector__ void write_kernel_unlock(
  ...
  asc_vf_call<write_key_vf<K>>(dim3{THREAD_NUM_2048}, n, keys,
                               d_dst_keys, thread_all, GetBlockIdx());
}
