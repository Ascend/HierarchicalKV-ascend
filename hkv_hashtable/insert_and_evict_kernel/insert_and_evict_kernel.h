...
__forceinline__ __simt_callee__ void copy_value(
 ...
 LAUNCH_BOUND(THREAD_NUM_512) inline void insert_and_evict_kernel_vf(
     ...
     uint32_t bucket_capacity, uint32_t dim,  __gm__ K* keys_addr_gm, __gm__ void* values_addr_gm,
     __gm__ S* scores_gm, S cur_score,  __gm__ K* evicted_keys_addr_gm, __gm__ void* evicted_values_addr_gm,
     __gm__ S* evicted_scores_addr_gm, __gm__ size_t* d_evicted_counter_addr_gm, uint64_t n, uint64_t thread_all,
   ...
   __gm__ K* __restrict__ keys =
       reinterpret_cast<__gm__ K*>(keys_addr_gm);
   __gm__ VecV* __restrict__ values = reinterpret_cast<__gm__ VecV*>(values_addr_gm);
   ...
   __gm__ VecV* __restrict__ evicted_values = reinterpret_cast<__gm__ VecV*>(evicted_values_addr_gm);
   ...
   for (uint64_t kv_idx = block_index * blockDim.x + threadIdx.x; kv_idx < n; kv_idx += thread_all) {
     ...
     __gm__ VecV* bucket_values = reinterpret_cast<__gm__ VecV*>(reinterpret_cast<__gm__ void*>(bucket->vectors));
     ...
     for (uint32_t offset = 0; offset < bucket_capacity; offset++) {
       ...
       // 3.1 找到key
       if (current_key == key) {
         K try_key = Simt::AtomicCas(current_key_ptr, current_key, static_cast<K>(LOCKED_KEY));
         // 抢占成功
         ...
         K try_key = Simt::AtomicCas(current_key_ptr, EMPTY_KEY, static_cast<K>(LOCKED_KEY));
         // 抢占成功
...
template <typename K = uint64_t, typename V = float, typename S = uint64_t,
 ...
 LAUNCH_BOUND(THREAD_NUM_512) inline void insert_and_evict_kernel_with_digest_vf(
     __gm__ Bucket<K, V, S>* buckets_addr_gm, __gm__ int32_t* buckets_size_addr_gm, uint64_t capacity,
...
__global__ __vector__ void insert_and_evict_kernel(
     ...
     uint64_t global_epoch, uint32_t value_size,
     uint32_t max_bucket_shift, uint64_t capacity_divisor_magic,
   ...
   DISPATCH_VALUE_SIZE(
           ...
           (Simt::VF_CALL<insert_and_evict_kernel_with_digest_vf<
                   ...
                   capacity_divisor_magic, capacity_divisor_shift,
                   n_align_warp, ub_shared_block_acc_mem, ub_shared_global_acc_mem, group_size)));
 }
 }  // namespace hkv
 }  // namespace npu
