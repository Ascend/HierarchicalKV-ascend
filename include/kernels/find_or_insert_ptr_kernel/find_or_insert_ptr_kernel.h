...
LAUNCH_BOUND(THREAD_NUM_512) inline void find_or_insert_ptr_kernel_vf(
    ...
    __gm__ const K* keys_gm, __gm__ void* value_ptrs_gm, __gm__ S* scores,
  ...
  for (uint64_t kv_idx = blockIdx * blockDim.x + threadIdx.x; kv_idx < n;
    ...
    score = ScoreFunctor::desired_when_missed(scores_ptr, kv_idx, global_epoch,
                                              cur_score);
    ...
    __gm__ VecV* bucket_values_ptr = reinterpret_cast<__gm__ VecV*>(
    ...
    for (int offset = 0; offset < bucket_capacity; offset++) {
      ...
      if (key_val == key) {
        ...
        ScoreFunctor::update_with_digest(
            bucket_keys_ptr, target_pos, scores_ptr, kv_idx, score,
            bucket_capacity, get_digest<K>(key), false);
        break;
      } else if (key_val == EMPTY_KEY) {
        // 3.2 找到空位，尝试插入
        K expected = EMPTY_KEY;
        K try_key =
            Simt::AtomicCas(reinterpret_cast<__gm__ K*>(current_key_ptr),
                            expected, static_cast<K>(LOCKED_KEY));
        if (try_key == expected) {
          target_pos = current_pos;
          *current_key_ptr = key;
          ScoreFunctor::update_with_digest(
              bucket_keys_ptr, target_pos, scores_ptr, kv_idx, score,
              bucket_capacity, get_digest<K>(key), true);
    ...
    if (target_pos == INVALID_KEY_POS) {
      ...
      for (uint32_t current_pos = 0; current_pos < bucket_capacity;
           current_pos++) {
        S current_score = bucket_scores_ptr[current_pos];
        if (current_score < min_score &&
            bucket_keys_ptr[current_pos] != LOCKED_KEY) {
      ...
      // 4.2 分数不足，无法准入
      if (score > min_score) {
        __gm__ K* current_key_ptr = bucket_keys_ptr + min_pos;
...
__global__ __vector__ void find_or_insert_ptr_kernel(
  ...
  DISPATCH_VALUE_SIZE(
      value_size,
      (Simt::VF_CALL<find_or_insert_ptr_kernel_vf<K, V, S, DTYPE, Strategy>>(
          Simt::Dim3{static_cast<uint32_t>(THREAD_NUM_512)}, buckets,
          buckets_size, buckets_num, bucket_capacity, dim, keys, value_ptrs,
          scores, key_ptrs, n, founds, global_epoch, cur_score, GetBlockIdx(),
          thread_all)));
}
}  // namespace hkv
}  // namespace npu
