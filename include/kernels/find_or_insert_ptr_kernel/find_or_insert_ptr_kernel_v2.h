      ...
      if (IS_RESERVED_KEY<K>(key)) {
        ...
        for (uint32_t offset = 0; offset < bucket_max_size + STRIDE;
          ...
          do {
            ...
            K try_key = Simt::AtomicCas(current_key_ptr, key,
...
LAUNCH_BOUND(THREAD_NUM_512) inline void find_or_insert_ptr_kernel_vf_v2(
    ...
    __gm__ const K* __restrict__ keys, __gm__ V* __gm__* __restrict__ vectors,
    __gm__ S* __restrict__ scores, S cur_score, __gm__ bool* __restrict__ found,
    const S global_epoch, const size_t n, uint32_t blockIdx,
    ...
    if (lane_id == 0) {
      ...
      if (occupy_result == OccupyResult::DUPLICATE) {
        ScoreFunctor::update_score_only(bucket_keys, key_pos, scores, key_idx,
                                        find_or_insert_score, bucket_max_size,
                                        false);
        vectors[key_idx] = (bucket_vectors + key_pos * dim);
      } else {
        ScoreFunctor::update_with_digest(
            bucket_keys, key_pos, scores, key_idx, find_or_insert_score,
