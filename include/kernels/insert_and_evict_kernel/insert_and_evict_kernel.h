...
__forceinline__ __simt_callee__ void copy_value(__gm__ const VecV* src,
                                                __gm__ VecV* dst,
                                                const uint32_t dim,
...
LAUNCH_BOUND(THREAD_NUM_512) inline void insert_and_evict_kernel_vf(
    __gm__ Bucket<K, V, S>* buckets_addr_gm, __gm__ int* buckets_size_addr_gm,
    uint64_t capacity, uint32_t bucket_capacity, uint32_t dim,
    __gm__ K* keys_addr_gm, __gm__ void* values_addr_gm, __gm__ S* scores_gm,
    S cur_score, __gm__ K* evicted_keys_addr_gm,
    __gm__ void* evicted_values_addr_gm, __gm__ S* evicted_scores_addr_gm,
    __gm__ size_t* d_evicted_counter_addr_gm, uint64_t n, uint64_t thread_all,
  ...
  __gm__ K* __restrict__ keys = reinterpret_cast<__gm__ K*>(keys_addr_gm);
  ...
  __gm__ VecV* __restrict__ evicted_values =
      reinterpret_cast<__gm__ VecV*>(evicted_values_addr_gm);
  ...
  for (uint64_t kv_idx = block_index * blockDim.x + threadIdx.x; kv_idx < n;
    ...
    __gm__ VecV* bucket_values = reinterpret_cast<__gm__ VecV*>(
    ...
    for (uint32_t offset = 0; offset < bucket_capacity; offset++) {
      ...
      __gm__ K* current_key_ptr = bucket_keys + current_pos;
      ...
      if (current_key == key) {
        ...
        K try_key = Simt::AtomicCas(current_key_ptr, EMPTY_KEY,
                                    static_cast<K>(LOCKED_KEY));
        // 抢占成功
    ...
    while (target_pos == INVALID_KEY_POS) {
      ...
      for (uint32_t current_pos = 0; current_pos < bucket_capacity;
        ...
        if (current_score < min_score && current_key != LOCKED_KEY &&
            current_key != EMPTY_KEY) {
      ...
      if (is_evicted) {
        for (uint32_t i = 0; i < dim; i++) {
          __stg<ST_L2CacheType::L2_CACHE_HINT_NORMAL_FV,
              ...
              evicted_values + evicted_idx * dim + i,
              bucket_values[target_pos * dim + i]);
        }
      }
...
LAUNCH_BOUND(THREAD_NUM_512) inline void insert_and_evict_non_unique_kernel_vf(
    ...
    __gm__ const K* keys, __gm__ const V* values_gm, __gm__ const S* scores,
    S cur_score, __gm__ K* evicted_keys, __gm__ V* evicted_values_gm,
    __gm__ S* evicted_scores, __gm__ size_t* d_evicted_counter, uint64_t n,
  ...
  __gm__ const VecV* values = reinterpret_cast<__gm__ const VecV*>(values_gm);
  ...
  for (uint64_t t = block_index * blockDim.x + threadIdx.x; t < N;
    ...
    if (occupy_result == OccupyResult::EVICT) {
      ...
      evicted_idx = asc_shfl(evicted_idx, 0, TILE_SIZE);
      copy_value<VecV, TILE_SIZE>(bucket_values + key_pos * dim,
                                  evicted_values + evicted_idx * dim, dim,
                                  lane_id);
    }

    copy_value<VecV, TILE_SIZE>(input_value, bucket_values + key_pos * dim, dim,
                                lane_id);
    asc_threadfence();
...
LAUNCH_BOUND(THREAD_NUM_512) inline void insert_and_evict_kernel_with_digest_vf(
    __gm__ Bucket<K, V, S>* buckets_addr_gm,
    __gm__ int32_t* buckets_size_addr_gm, uint64_t capacity,
    uint32_t bucket_max_size, uint32_t dim, __gm__ K* keys_addr_gm,
    __gm__ void* values_addr_gm, __gm__ S* scores_gm, S cur_score,
    __gm__ K* evicted_keys_addr_gm, __gm__ void* evicted_values_addr_gm,
    __gm__ S* evicted_scores_addr_gm, __gm__ size_t* d_evicted_counter_addr_gm,
  ...
  for (uint64_t kv_idx = block_index * blockDim.x + threadIdx.x;
    ...
    int32_t rank = threadIdx.x % COUNT_GROUP_SIZE;
    ...
    uint32_t group_evict_count =
        __shfl(prefix_sum, COUNT_GROUP_SIZE - 1, COUNT_GROUP_SIZE);
    ...
    for (int32_t i = 0; i < group_size; i++) {
      int32_t occupy_result_cur = __shfl(occupy_result, i, group_size);
      if (occupy_result_cur == OccupyResult::ILLEGAL) {
        continue;
      }

      int32_t kv_idx_cur = __shfl(kv_idx, i, group_size);
      int32_t evicted_idx_cur = __shfl(evicted_idx, i, group_size);
      int32_t key_pos_cur = __shfl(key_pos, i, group_size);
      uint64_t bucket_values_cur = __shfl(bucket_values_uintptr, i, group_size);

      __gm__ VecV* bucket_values_ptr =
          reinterpret_cast<__gm__ VecV*>(bucket_values_cur);
...
__global__ __vector__ void insert_and_evict_non_unique_kernel(
    ...
    __gm__ K* evicted_keys, __gm__ V* evicted_values, __gm__ S* evicted_scores,
    __gm__ size_t* d_evicted_counter, uint64_t n, uint64_t global_epoch,
  ...
  DISPATCH_VALUE_SIZE(
      value_size,
      (asc_vf_call<
          insert_and_evict_non_unique_kernel_vf<K, V, S, DTYPE, Strategy>>(
          ...
          evicted_scores, d_evicted_counter, n, thread_all, global_epoch,
...
__global__ __vector__ void insert_and_evict_kernel(
    ...
    __gm__ void* values, __gm__ S* scores, __gm__ K* evicted_keys,
    __gm__ void* evicted_values, __gm__ S* evicted_scores,
    __gm__ size_t* d_evicted_counter, uint64_t n, uint64_t global_epoch,
    uint32_t value_size, uint32_t max_bucket_shift,
    uint64_t capacity_divisor_magic, uint64_t capacity_divisor_shift,
  ...
  DISPATCH_VALUE_SIZE(
      value_size,
      (Simt::VF_CALL<
          insert_and_evict_kernel_with_digest_vf<K, V, S, DTYPE, Strategy>>(
          Simt::Dim3{static_cast<uint32_t>(THREAD_NUM_512)}, buckets,
          buckets_size, capacity, bucket_max_size, dim, keys, values, scores,
          cur_score, evicted_keys, evicted_values, evicted_scores,
          d_evicted_counter, n, thread_all, global_epoch, GetBlockIdx(),
