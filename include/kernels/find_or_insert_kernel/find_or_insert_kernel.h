...
/* !
 * \file find_or_insert_kernel.h
 * \brief find_or_insert_kernel
 */

 #ifndef ASCENDC_FIND_OR_INSERT_KERNEL_H_
 #define ASCENDC_FIND_OR_INSERT_KERNEL_H_
 ...
 #include "../insert_or_assign_kernel/insert_or_assign_kernel.h"
 ...
 template <typename K = uint64_t, typename V = float, typename S = uint64_t,
           typename VecV = int32_t, int32_t CG_SIZE = 16, int Strategy = -1,
           int32_t EVICT_GROUP_SIZE = 16>
 __simt_vf__ __aicore__
 LAUNCH_BOUND(THREAD_NUM_512) inline void find_or_insert_kernel_vf(
     __gm__ Bucket<K, V, S>* __restrict__ buckets,
     __gm__ int32_t* __restrict__ buckets_size, uint32_t bucket_max_size,
     uint32_t dim, __gm__ const K* __restrict__ keys,
     __gm__ V* __restrict__ values, __gm__ S* __restrict__ scores, uint64_t n,
     ...
     uint64_t capacity) {
   if ((!buckets) || (!buckets_size) || (!keys) || (!values)) {
     return;
   }
   ...
   __gm__ VecV* __restrict__ values_vecv =
       reinterpret_cast<__gm__ VecV*>(values);
   S score = static_cast<S>(EMPTY_SCORE);
   constexpr uint32_t STRIDE = sizeof(VecD_Comp) / sizeof(D);
   const size_t n_align_warp = ((n + warpSize - 1) / warpSize) * warpSize;

   uint32_t key_pos = 0;
   K key = 0;
   __gm__ K* bucket_keys = nullptr;
   uint64_t bucket_values = 0;
   ...
   for (uint64_t kv_idx = blockIdx * blockDim.x + threadIdx.x;
     ...
     if (kv_idx < n) {
       ...
       if (IS_RESERVED_KEY<K>(key)) {
         ...
         bucket_values = reinterpret_cast<uint64_t>(bucket->vectors);
         ...
         for (uint32_t offset = 0; offset < bucket_max_size + STRIDE;
           ...
           do {
             ...
             __gm__ K* current_key_ptr =
                 BUCKET::keys(bucket_keys, possible_pos);
     ...
     for (int32_t i = 0; i < EVICT_GROUP_SIZE; i++) {
       int32_t res_sync = asc_shfl(occupy_result, i, EVICT_GROUP_SIZE);
         ...
         if (cg_rank_id == i) {
           ...
           if (score < min_score) {
             ...
             if (current_key != static_cast<K>(LOCKED_KEY) &&
                 current_key != static_cast<K>(EMPTY_KEY)) {
               K try_key = asc_atomic_cas(current_key_ptr, current_key,
                                           static_cast<K>(LOCKED_KEY));
               // 4.4 抢占成功
     ...
     // 5. 抢占成功，写入value
     cg_rank_id = threadIdx.x % CG_SIZE;
     for (int i = 0; i < CG_SIZE; i++) {
       int32_t res_sync = asc_shfl(occupy_result, i, CG_SIZE);
       if ((res_sync != OccupyResult::REFUSED) &&
           (res_sync != OccupyResult::ILLEGAL)) {
         auto kv_idx_sync = asc_shfl(kv_idx, i, CG_SIZE);
         auto key_pos_sync = asc_shfl(key_pos, i, CG_SIZE);
         uint64_t bucket_values_sync = asc_shfl(bucket_values, i, CG_SIZE);
         auto value_start = kv_idx_sync * dim;
         auto bucket_value_start = key_pos_sync * dim;
         if (res_sync == OccupyResult::DUPLICATE) {
           __gm__ VecV* bucket_values_vec =
               reinterpret_cast<__gm__ VecV*>(bucket_values_sync);
           for (uint32_t j = cg_rank_id; j < dim; j += CG_SIZE) {
             __stg<ST_L2CacheType::L2_CACHE_HINT_NORMAL_FV,
                   L1CacheType::NON_CACHEABLE>(
                 values_vecv + value_start + j,
                 bucket_values_vec[bucket_value_start + j]);
           }
         } else {
           for (uint32_t j = cg_rank_id; j < dim; j += CG_SIZE) {
             __stg<ST_L2CacheType::L2_CACHE_HINT_NORMAL_FV,
 ...
 template <class K, class V, class S, int32_t CG_SIZE = 16, int Strategy = -1>
 __global__ __vector__ void find_or_insert_kernel(
     __gm__ Bucket<K, V, S>* buckets, __gm__ int32_t* buckets_size,
     uint32_t bucket_capacity, uint32_t dim, __gm__ const K* keys,
     __gm__ V* values, __gm__ S* scores, uint64_t n, uint64_t global_epoch,
   ...
   DISPATCH_VALUE_SIZE(
       value_size,
       (asc_vf_call<find_or_insert_kernel_vf<K, V, S, DTYPE, CG_SIZE, Strategy>>(
           dim3{static_cast<uint32_t>(THREAD_NUM_512)}, buckets, buckets_size,
           bucket_capacity, dim, keys, values, scores, n, global_epoch,
 ...
 __simt_vf__ __aicore__
 LAUNCH_BOUND(THREAD_NUM_512) inline void find_or_insert_hybrid_kernel_vf(
     __gm__ Bucket<K, V, S>* __restrict__ buckets,
     __gm__ int32_t* __restrict__ buckets_size, uint32_t bucket_max_size,
     uint32_t dim, __gm__ const K* __restrict__ keys,
     __gm__ V* __restrict__ values, __gm__ S* __restrict__ scores,
     __gm__ V * __gm__ * __restrict__ d_dst_values,
     __gm__ K * __gm__ * __restrict__ d_dst_keys,
     __gm__ bool* founds, uint64_t n, uint64_t global_epoch,
     uint64_t cur_score, uint32_t blockIdx, uint64_t thread_all,
     uint32_t max_bucket_shift, uint64_t capacity_divisor_magic,
     uint64_t capacity_divisor_shift, uint64_t capacity) {
   if ((!buckets) || (!buckets_size) || (!keys) || (!values) || (!d_dst_keys) ||
       (!d_dst_values) || (!founds)) {
     return;
   }
 ...
 __global__ __vector__ void find_or_insert_hybrid_kernel(
     __gm__ Bucket<K, V, S>* buckets, __gm__ int32_t* buckets_size,
     uint32_t bucket_capacity, uint32_t dim, __gm__ const K* keys,
     __gm__ V* values, __gm__ S* scores, __gm__ V * __gm__ * d_dst_values,
     __gm__ K * __gm__ * d_dst_keys, __gm__ bool* founds, uint64_t n,
     uint64_t global_epoch, uint32_t max_bucket_shift,
     uint64_t capacity_divisor_magic, uint64_t capacity_divisor_shift,
     uint64_t capacity) {
   const uint64_t thread_all = THREAD_NUM_512 * GetBlockNum();
   ...
   asc_vf_call<find_or_insert_hybrid_kernel_vf<K, V, S, Strategy>>(
       dim3{static_cast<uint32_t>(THREAD_NUM_512)}, buckets, buckets_size,
       bucket_capacity, dim, keys, values, scores, d_dst_values, d_dst_keys,
       founds, n, global_epoch, cur_score, GetBlockIdx(), thread_all,
       max_bucket_shift, capacity_divisor_magic, capacity_divisor_shift,
       capacity);
 }

 template <class K, class V, bool UNLOCK_KEY>
 __global__ __vector__ void read_or_write_value_kernel(
     uint32_t former_num, uint64_t former_core_move_num,
     uint64_t tail_core_move_num, uint32_t tile_size, uint32_t num_tiles,
     uint32_t dim, __gm__ K* keys, __gm__ V* param_values, uint64_t n,
     __gm__ V * __gm__ * table_value_addrs, __gm__ K * __gm__ * d_dst_keys,
     __gm__ bool* founds) {
   ...
   for (uint64_t i = core_start_idx; i < core_start_idx + core_move_count;
     ...
     for (uint32_t tile_idx = 0; tile_idx < num_tiles; tile_idx++) {
       ...
       table_values_addr_gm.SetGlobalBuffer(table_value_addr + tile_idx * tile_size);

       if (founds[i]) {
         // 找到 key，从 table 中读取值返回给用户
         ...
         DataCopyPad(param_values_gm[dst_offset + tile_idx * tile_size],
                     move_local, copy_params);
       } else {
         // 没找到 key，把用户的值写入 table
         DataCopyPad(move_local, param_values_gm[dst_offset + tile_idx * tile_size], copy_params, pad_params);
         move_queue.EnQue<V>(move_local);
         move_queue.DeQue<V>(move_local);
 ...
 #endif  // ASCENDC_FIND_OR_INSERT_KERNEL_H_
