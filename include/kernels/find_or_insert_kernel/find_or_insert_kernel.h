...
LAUNCH_BOUND(THREAD_NUM_512) inline void find_or_insert_non_unique_kernel_vf(
    ...
    __gm__ V* values, __gm__ V * __gm__ * d_dst_values, __gm__ bool* founds,
...
__global__ __vector__ void find_or_insert_non_unique_kernel(
  ...
  asc_vf_call<find_or_insert_non_unique_kernel_vf<K, V, S, MoveV, Strategy>>(
      ...
      d_dst_values, founds, thread_all, global_epoch, GetBlockIdx(),
...
LAUNCH_BOUND(THREAD_NUM_512) inline void find_or_insert_hybrid_kernel_vf(
    ...
    __gm__ K * __gm__ * __restrict__ d_dst_keys, __gm__ bool* founds,
    uint64_t n, uint64_t global_epoch, uint64_t cur_score, uint32_t blockIdx,
    uint64_t thread_all, uint32_t max_bucket_shift,
    uint64_t capacity_divisor_magic, uint64_t capacity_divisor_shift,
    uint64_t capacity) {
    ...
    for (uint32_t tile_idx = 0; tile_idx < num_tiles; tile_idx++) {
      ...
      if (founds[i]) {
        ...
        DataCopyPad(move_local,
                    param_values_gm[dst_offset + tile_idx * tile_size],
