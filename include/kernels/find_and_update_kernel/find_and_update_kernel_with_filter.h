 ...
 */
 ...
 LAUNCH_BOUND(THREAD_NUM_1024) inline void find_and_update_kernel_with_filter_vf(
     __gm__ Bucket<K, V, S>* buckets_gm, uint64_t capacity, uint32_t bucket_capacity,
     uint32_t dim, __gm__ K* keys_gm, __gm__ V * __gm__* value_ptrs_gm, __gm__ S* scores_gm,
     __gm__ bool* founds_gm, uint64_t n, bool update_score, uint64_t global_epoch,
   ...
   for (; kv_idx < n; kv_idx += total_thread_num) {
     ...
     for (uint32_t offset = 0; offset < bucket_capacity + STRIDE && !done;
       ...
       while (cmp_result != 0 && !done) {
         ...
         K try_key = AscendC::Simt::AtomicCas(current_key_ptr, key, static_cast<K>(LOCKED_KEY));
         if (try_key == key) {
 ...
 __global__ __vector__ void find_and_update_kernel_with_filter(
     __gm__ Bucket<K, V, S>* buckets, uint64_t capacity, uint32_t bucket_capacity, uint32_t dim,
     __gm__ K* keys,  __gm__ V * __gm__* value_ptrs, __gm__ S* scores, __gm__ bool* founds,
   ...
   Simt::VF_CALL<find_and_update_kernel_with_filter_vf<K, V, S, Strategy>>(
       Simt::Dim3{static_cast<uint32_t>(THREAD_NUM_1024)}, buckets, capacity,
