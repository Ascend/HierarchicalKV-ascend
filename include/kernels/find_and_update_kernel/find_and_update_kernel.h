...
LAUNCH_BOUND(THREAD_NUM_512) inline void find_and_update_kernel_vf(
  ...
  for (; kv_idx < n; kv_idx += total_thread_num) {
    ...
    if (!IS_RESERVED_KEY<K>(key)) {
      ...
      for (; offset < bucket_capacity; offset++) {
        ...
        if (current_key == key) {
          K try_key = Simt::AtomicCas(current_key_ptr, current_key, LOCKED_KEY);
          // 抢占成功
...
__global__ __vector__ void find_and_update_kernel(
  ...
  Simt::VF_CALL<find_and_update_kernel_vf<K, V, S, Strategy>>(
