...
LAUNCH_BOUND(THREAD_NUM_512) inline void find_ptr_kernel_vf(
    GM_ADDR buckets_gm, uint64_t buckets_num, uint32_t bucket_capacity,
    uint32_t dim, GM_ADDR keys_gm, GM_ADDR value_ptrs_gm, GM_ADDR scores_gm,
    GM_ADDR founds_gm, uint64_t n, const uint64_t total_thread_num,
...
__global__ __vector__ void find_ptr_kernel(
    GM_ADDR buckets, uint64_t buckets_num, uint32_t bucket_capacity,
    uint32_t dim, GM_ADDR keys, GM_ADDR value_ptrs, GM_ADDR scores,
    GM_ADDR founds, uint64_t n, uint64_t global_epoch) {
  const uint64_t thread_all = THREAD_NUM_512 * GetBlockNum();

  Simt::VF_CALL<find_ptr_kernel_vf<K, V, S>>(
      Simt::Dim3{static_cast<uint32_t>(THREAD_NUM_512), 1, 1}, buckets,
      buckets_num, bucket_capacity, dim, keys, value_ptrs, scores, founds, n,
