...
LAUNCH_BOUND(THREAD_NUM_512) inline void find_ptr_with_digest_kernel_vf(
    __gm__ Bucket<K, V, S>* buckets_gm, uint64_t capacity, uint64_t buckets_num,
    uint32_t bucket_capacity, uint32_t dim, __gm__ K* keys_gm,
    __gm__ V* __gm__* value_ptrs_gm, __gm__ S* scores_gm,
    __gm__ bool* founds_gm, uint64_t n, const uint64_t total_thread_num,
  ...
  for (uint64_t kv_idx = block_id * blockDim.x + threadIdx.x; kv_idx < n;
       kv_idx += total_thread_num) {
...
__global__ __vector__ void find_ptr_with_digest_kernel(
    __gm__ Bucket<K, V, S>* buckets, uint64_t capacity, uint64_t buckets_num,
    uint32_t bucket_capacity, uint32_t dim, __gm__ K* keys,
    __gm__ V* __gm__* value_ptrs, __gm__ S* scores, __gm__ bool* founds,
    uint64_t n, uint64_t global_epoch, uint32_t max_bucket_shift,
    uint64_t capacity_divisor_magic, uint64_t capacity_divisor_shift) {
  const uint64_t thread_all = THREAD_NUM_512 * GetBlockNum();

  Simt::VF_CALL<find_ptr_with_digest_kernel_vf<K, V, S>>(
      Simt::Dim3{static_cast<uint32_t>(THREAD_NUM_512), 1, 1}, buckets,
      capacity, buckets_num, bucket_capacity, dim, keys, value_ptrs, scores,
      founds, n, thread_all, global_epoch, GetBlockIdx(), max_bucket_shift,
