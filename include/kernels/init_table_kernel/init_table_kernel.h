...
__simt_vf__ __aicore__ LAUNCH_BOUND(1) inline void allocate_bucket_vectors_vf(
    __gm__ Bucket<K, V, S>* buckets_gm, const size_t index,
    __gm__ V* address_gm) {
  __gm__ Bucket<K, V, S>* buckets =
      reinterpret_cast<__gm__ Bucket<K, V, S>* __restrict>(buckets_gm);
...
__simt_vf__ __aicore__ LAUNCH_BOUND(1) inline void allocate_bucket_others_vf(
    __gm__ Bucket<K, V, S>* buckets_gm, size_t total_size_per_bucket,
    size_t num_of_buckets, const int start_index, __gm__ uint8_t* address,
    const uint32_t reserve_size, const size_t bucket_max_size) {
  __gm__ Bucket<K, V, S>* buckets =
      reinterpret_cast<__gm__ Bucket<K, V, S>* __restrict>(buckets_gm);
...
__simt_vf__ __aicore__ LAUNCH_BOUND(1) inline void get_bucket_others_address_vf(
    __gm__ Bucket<K, V, S>* buckets_gm, const int index,
    __gm__ uint8_t* __gm__* address_gm) {
  __gm__ Bucket<K, V, S>* buckets =
      reinterpret_cast<__gm__ Bucket<K, V, S>* __restrict>(buckets_gm);
  __gm__ uint8_t* __gm__* address =
      reinterpret_cast<__gm__ uint8_t * __gm__*>(address_gm);
...
LAUNCH_BOUND(THREAD_NUM_512) inline void create_atomic_keys_vf(
    __gm__ Bucket<K, V, S>* buckets_gm, const size_t start, const size_t end,
    const size_t bucket_max_size, uint32_t blockIdx) {
  __gm__ Bucket<K, V, S>* buckets =
      reinterpret_cast<__gm__ Bucket<K, V, S>* __restrict>(buckets_gm);
...
__global__ __vector__ void allocate_bucket_others_kernel(
  ...
  Simt::VF_CALL<allocate_bucket_others_vf<K, V, S>>(
      Simt::Dim3{static_cast<uint32_t>(1)}, buckets_gm, total_size_per_bucket,
      num_of_buckets, start_index, address, reserve_size, bucket_max_size);
}
...
__global__ __vector__ void create_atomic_keys_kernel(
  ...
  Simt::VF_CALL<create_atomic_keys_vf<K, V, S>>(
      Simt::Dim3{static_cast<uint32_t>(THREAD_NUM_512)}, buckets_gm, start, end,
      bucket_max_size, GetBlockIdx());
}
