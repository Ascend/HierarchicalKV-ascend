...
/* !
 * \file dump_kernel_if.h
 * \brief dump_kernel_if
 */

#ifndef ASCENDC_DUMP_KERNEL_IF_H_
#define ASCENDC_DUMP_KERNEL_IF_H_
...
template <class K, class V, class S, class VecV,
          template <typename, typename> class PredFunctor, int32_t GROUP_SIZE>
__simt_vf__ __aicore__
LAUNCH_BOUND(THREAD_NUM_2048) inline void dump_kernel_if_vf(
    __gm__ Table<K, V, S>* table, __gm__ Bucket<K, V, S>* buckets,
    const K pattern, const S threshold, __gm__ K* d_key, __gm__ V* d_val,
  ...
  const size_t search_length_align =
      ((search_length + warpSize - 1) / warpSize) * warpSize;

  auto block_tuples = reinterpret_cast<__ubuf__ KVM<K, S>*>(ub_shared_kvs_mem);
  ...
  for (; tid < search_length_align; tid += total_thread_num) {
    ...
    if (tid < search_length) {
      ...
      const __gm__ V* value_ptrs =
          reinterpret_cast<__gm__ V*>(&bucket_values_ptr[key_idx * dim_in]);
      ...
      if (match) {
        ...
        block_tuples[local_index].value =
            reinterpret_cast<uint64_t>(value_ptrs);
        block_tuples[local_index].score = score;
      }
      asc_syncthreads();

      if (threadIdx.x == 0) {
        global_acc[0] =
            asc_atomic_add(dump_counter, static_cast<size_t>(block_acc[0]));
      }
      asc_syncthreads();
...
template <class K, class V, class S,
          template <typename, typename> class PredFunctor, int32_t GROUP_SIZE>
__global__ __vector__ void dump_kernel_if(
    __gm__ Table<K, V, S>* table, __gm__ Bucket<K, V, S>* buckets,
    const K pattern, const S threshold, __gm__ K* keys, __gm__ V* vals,
  ...
  pipe.InitBuffer(global_acc, sizeof(size_t));
  AscendC::LocalTensor<size_t> shared_global_acc_tensor =
      global_acc.Get<size_t>();
  __ubuf__ size_t* ub_shared_global_acc_mem =
      reinterpret_cast<__ubuf__ size_t*>(shared_global_acc_tensor.GetPhyAddr());

  DISPATCH_VALUE_SIZE(
      value_size,
      (asc_vf_call<dump_kernel_if_vf<K, V, S, DTYPE, PredFunctor, GROUP_SIZE>>(
          dim3{static_cast<uint32_t>(THREAD_NUM_2048)}, table, buckets, pattern,
          threshold, keys, vals, scores, offset, search_length,
...
#endif  // ASCENDC_DUMP_KERNEL_IF_H_
