...
#ifndef ASCENDC_DUMP_KERNEL_IF_V2_H_
#define ASCENDC_DUMP_KERNEL_IF_V2_H_
...
LAUNCH_BOUND(THREAD_NUM_2048) inline void dump_kernel_if_v2_vf(
  ...
  for (; tid < search_length_align; tid += total_thread_num) {
    ...
    if (tid < search_length) {
      ...
      const S score = bucket_scores_ptr[key_idx];
      ...
      if (threadIdx.x == 0) {
        global_acc[0] =
            atomicAdd(dump_counter, static_cast<size_t>(block_acc[0]));
      }
      asc_syncthreads();
...
__global__ __vector__ void dump_kernel_if_v2(
    PredFunctor pred, __gm__ Table<K, V, S>* table,
  ...
  DISPATCH_VALUE_SIZE(
      ...
      (asc_vf_call<
          ...
          dump_counter, shared_kvs_tensor.GetPhyAddr(), ub_shared_block_acc_mem,
